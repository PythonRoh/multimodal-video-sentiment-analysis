...
export function Inference({ quota }: InferenceProps) {
  ...
  return (
    <div className="flex h-fit w-full flex-col gap-3 md:w-1/2">
        ...
        <div className="flex h-fit w-full flex-wrap items-center justify-center gap-4 rounded-xl border border-gray-200 p-4 sm:gap-8 sm:px-6">
          <div className="flex flex-col items-center">
            ...
            <span className="text-[40px]">
              {averages.topEmotion ? EMOTION_EMOJI[averages.topEmotion.label] : "😐"}
            </span>
            <span className="text-sm text-gray-500">
              {averages.topEmotion?.confidence.toFixed(3)} (
              {((averages.topEmotion?.confidence ?? 0) * 100).toFixed(0)}%)
            </span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-sm">Primary Sentiment</span>
            <span className="text-[40px]">
              {averages.topSentiment ? SENTIMENT_EMOJI[averages.topSentiment.label] : "😐"}
            </span>
            <span className="text-sm text-gray-500">
              {averages.topSentiment?.confidence.toFixed(3)} (
              {((averages.topSentiment?.confidence ?? 0) * 100).toFixed(0)}%)
            </span>
          </div>
        </div>
      ) : (
      ...
      {analysis?.utterances && analysis.utterances.length > 0 ? (
      ...
      ) : analysis?.utterances?.length === 0 ? (
        <div className="flex h-32 w-full items-center justify-center rounded-xl border border-dashed border-red-200 bg-red-50 p-4">
          <span className="text-sm text-red-400">
            No speech detected in this video. Please upload a video with clear dialogue.
          </span>
        </div>
      ) : (
        <div className="flex h-32 w-full items-center justify-center rounded-xl border border-dashed border-gray-200 p-4">
